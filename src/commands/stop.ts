...
class StopCommand implements Command {
  readonly name = 'stop';
  readonly description = 'Stop the remote service';
  readonly usage = 'shipway stop [service]';
  ...
  async execute(ctx: CommandContext): Promise<number> {
    ...
    if (ctx.config.services && Object.keys(ctx.config.services).length > 0) {
      ...
      for (const [svcName, svc] of entries) {
        ...
        ctx.logger.info(`Stopping ${name}...`);
        await pm.stop(ssh, name);
        ctx.logger.success(`${name} stopped`);
      }
      return ExitCode.OK;
    }
